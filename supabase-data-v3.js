...
;(function () {
	...
	var _certLogoSrc = 'assets/1.jpeg'

	function _loadCertLogo(cb) {
		console.log('[CERT] _loadCertLogo called')
		_certLogoOK = false
		var img = new Image()
		img.onload = function() {
			console.log('[CERT] Logo loaded OK, size:', img.naturalWidth, 'x', img.naturalHeight)
			_certLogo = img
			_certLogoOK = true
			if (cb) cb()
		}
		img.onerror = function(e) {
			console.error('[CERT] Logo load FAILED:', e)
			_certLogoOK = false
			if (cb) cb()
		}
		// Cache-busting agresif: timestamp unik tiap pemanggilan
		img.src = _certLogoSrc + '?cb=' + Date.now() + Math.random()
	}
	...
	function drawAdopsiCert(cv, data) {
		try {
			...
			if (_certLogoOK && _certLogo) {
				ctx.drawImage(_certLogo, cx - 85, logoY - 85, 170, 170)
			} else {
				ctx.beginPath()
			...
			ctx.fillText('ORGANISASI PENCINTA ALAM', cx, logoY + 120)

			ctx.fillStyle = '#d4af37'
			ctx.font = 'bold 36px Georgia, serif'
			ctx.fillText('RCS.CBS', cx, logoY + 160)

			ctx.fillStyle = '#d4af37'
			ctx.font = 'bold 80px Georgia, serif'
			ctx.fillText('SERTIFIKAT ADOPSI POHON', cx, logoY + 260)

			// === INTRO ===
			var introY = logoY + 330
			...
			var sealY = sigY + 130
			if (_certLogoOK && _certLogo) {
				ctx.drawImage(_certLogo, cx - 55, sealY - 55, 110, 110)
			} else {
	...
	function drawExampleCert() {
		...
		console.log('[CERT] About to call _loadCertLogo');
		console.log('[CERT] _loadCertLogo exists?', typeof _loadCertLogo);
		console.log('[CERT] _certLogoOK current value:', _certLogoOK);
		console.log('[CERT] _certLogo current value:', _certLogo);
		_loadCertLogo(function() {
			console.log('[CERT] Logo loaded, now drawing cert');
